...
def read_labels():
    ...
    with open(r"video refs\labels.txt", "r") as fr:
        ...
        for line in fr:
            ...
            lower, upper = int(round(float(tokens[0]) * 100)), int(round(float(tokens[1]) * 100))
...
def create_video(files_and_duration):
    ...
    for file, duration in tqdm(files_and_duration):
        ...
        print(duration, duration * frame_per_second, int(round(duration*frame_per_second)))

        # Repating the frame to fill the duration
        for repeat in range(int(round(duration * frame_per_second, 2))): # might be the problem
            writer.write(frame)
